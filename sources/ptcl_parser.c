...
static ptcl_parse_arguments_result ptcl_parser_parse_arguments(ptcl_parser *parser, ptcl_location location, bool with_self, bool with_names, bool with_variadic)
{
    ...
    while (!ptcl_parser_match(parser, ptcl_token_right_par_type))
    {
        ...
        else
        {
            ...
            ptcl_type type = ptcl_parser_type(parser, false, false, false);
            if (ptcl_parser_critical(parser))
            {
                ptcl_name_destroy(argument_name);
                goto leave;
...
ptcl_statement *ptcl_parser_parse_statement(ptcl_parser *parser)
{
    ...
    if (is_finded)
    {
        ...
        case ptcl_statement_func_decl_type:
            statement->func_decl = ptcl_parser_func_decl(
                parser,
                ptcl_parser_match(parser, ptcl_token_prototype_type),
                ptcl_parser_match(parser, ptcl_token_global_type),
                ptcl_parser_match(parser, ptcl_token_static_type));
            break;
        case ptcl_statement_typedata_decl_type:
            statement->typedata_decl = ptcl_parser_typedata_decl(
                parser,
                ptcl_parser_match(parser, ptcl_token_prototype_type),
                ptcl_parser_match(parser, ptcl_token_global_type));
            break;
        case ptcl_statement_type_decl_type:
            statement->type_decl = ptcl_parser_type_decl(
                parser,
                ptcl_parser_match(parser, ptcl_token_auto_type),
                ptcl_parser_match(parser, ptcl_token_prototype_type),
                ptcl_parser_match(parser, ptcl_token_global_type));
            break;
                                                                          ...
                                                                          ptcl_statement_func_call *func_call, ptcl_expression *self, ptcl_location location)
{
    ...
    *placeholder = ptcl_statement_func_body_inserted_create(NULL, 0, parser->root, target->func.arguments, target->func.count, self->return_type.is_static ? NULL : self, *func_call);
    ...
    for (size_t i = 0; i < func_call->count; i++)
    {
        ...
        ptcl_parser_variable variable = ptcl_parser_variable_create(target->func.name, argument->return_type, argument, argument->return_type.is_static, placeholder);
        if (!ptcl_parser_add_instance_variable(parser, variable))
        {
            ptcl_statement_func_call_destroy(*func_call);
            free(placeholder);
    ...
    const bool last_state = parser->is_except_return;
    const ptcl_type *last_return_type = parser->return_type;
...
ptcl_statement_func_decl ptcl_parser_func_decl(ptcl_parser *parser, bool is_prototype, bool is_global, bool is_static)
{
    ...
    if (ptcl_parser_try_get_function(parser, name, &prototype_function) && !prototype_function->func.is_prototype)
    {
...
ptcl_statement_type_decl ptcl_parser_type_decl(ptcl_parser *parser, bool is_auto, bool is_prototype, bool is_global)
{
    ...
    *comp_type = ptcl_type_create_comp_type(decl, is_static);
    ptcl_parser_comp_type instance = ptcl_parser_comp_type_create(
        is_global ? NULL : ptcl_parser_root(parser),
        name,
        comp_type,
        comp_type->is_static,
        is_auto && comp_type->is_static);

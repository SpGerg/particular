...
static ptcl_statement *ptcl_create_statement_by_ctor(ptcl_statement_func_body *root, ptcl_expression *expression, ptcl_location location)
{
    ptcl_statement *statement = ptcl_statement_create(
        (ptcl_statement_type)expression->ctor.values[PTCL_PARSER_STATEMENT_TYPEDATA_TYPE_I]->integer_n,
        root,
...
static ptcl_expression *ptcl_create_ctor_from_statement(ptcl_statement *statement, ptcl_location location)
{
    ...
    ptcl_expression **values = malloc(PTCL_PARSER_STATEMENT_TYPEDATA_COUNT * sizeof(ptcl_expression *));
    if (values == NULL)
    {
        free(expression);
        return NULL;
    ...
    ptcl_expression *statement_type = ptcl_expression_create(
    ...
    statement_type->integer_n = (int)statement->type;
    values[0] = statement_type;
    expression->ctor = ptcl_expression_ctor_create(
        ptcl_statement_t_name,
        values,
        PTCL_PARSER_STATEMENT_TYPEDATA_COUNT);
    return expression;
}
...
static ptcl_expression *ptcl_get_statements_realization(ptcl_parser *parser, ptcl_expression **arguments, size_t count, ptcl_location location, bool is_expression)
{
    ...
    case ptcl_expression_func_body_type:
        ...
        for (size_t i = 0; i < func_body.count; i++)
        {
            ...
            if (expression == NULL)
            {
                ...
                if (i > 0)
                {
                    ...
                    {
                        ptcl_expression_destroy(statements[i]);
                    }
        ...
        result->array.count = func_body.count;
        result->array.expressions = statements;
        result->array.type = ptcl_statement_t_type;
        break;
    }
...
static ptcl_expression *ptcl_insert_realization(ptcl_parser *parser, ptcl_expression **arguments, size_t count, ptcl_location location, bool is_expression)
{
    ...
    switch (argument->return_type.type)
    {
    case ptcl_value_array_type:
        ...
        case ptcl_value_typedata_type:
            if (ptcl_name_compare(target.typedata, ptcl_statement_t_name))
            {
                ptcl_statement_func_body *body;
                if (parser->syntax_depth > 0)
                {
                    body = parser->main_root;
                }
                else
                {
                    body = parser->root;
                ...
                for (size_t i = 0; i < array.count; i++)
                {
                    ptcl_statement *copy = ptcl_create_statement_by_ctor(body, array.expressions[i], location);
                    if (copy == NULL)
                    {
...
ptcl_parser_result ptcl_parser_parse(ptcl_parser *parser)
{
    ...
    ptcl_typedata_builder statement_builder = ptcl_typedata_builder_create(PTCL_PARSER_STATEMENT_TYPEDATA_NAME);
    ptcl_type statement_type = ptcl_type_create_typedata(statement_builder.name, false);
    ptcl_typedata_builder_add_member(&statement_builder, "statement_type", ptcl_type_integer);
    ptcl_parser_add_instance(parser, ptcl_typedata_builder_build(statement_builder));
...
ptcl_statement *ptcl_parser_parse_statement(ptcl_parser *parser)
{
    ...
    if (parser->is_syntax_body && ptcl_parser_parse_try_parse_syntax_usage_here(parser, true))
    {
        size_t stop = parser->position;
        parser->position = start;

        ptcl_statement_func_body *body = parser->main_root;
        ptcl_parser_parse_extra_body(parser, true);
    ...
    if (is_finded)
    {
        ...
        case ptcl_statement_func_call_type:
            statement->func_call = ptcl_parser_parse_func_call(parser, NULL, false);
...
static bool ptcl_parser_insert_syntax(
    ...
    bool is_statement)
{
    ptcl_token *original_tokens = malloc(syntax->tokens_count * sizeof(ptcl_token));
    ...
    {
        original_tokens[i] = ptcl_token_same(parser->tokens[syntax->start + i]);
    }

    size_t tokens_to_insert_count = syntax->tokens_count;
    if (is_statement)
    {
...
{
    return ptcl_parser_parse_try_parse_syntax_usage(parser, NULL, 0, -1, false, is_statement);
}
    ...
    ptcl_parser *parser, ptcl_parser_syntax_node **nodes, size_t count, int down_start, bool skip_first, bool is_statement)
{
    ...
    while (!ptcl_parser_ended(parser))
    {
        if (!skip_first)
        {
            ...
            if (ptcl_parser_parse_try_parse_syntax_usage(parser, &syntax.nodes, syntax.count, start, true, is_statement))
            {
                return true;
            }
    ...
    if (found)
    {
        ...
        *temp = ptcl_statement_func_body_create(NULL, 0, parser->root);
        ...
        ptcl_parser_insert_syntax(parser, &result, start - 1, stop, is_statement);
...
ptcl_statement_func_call ptcl_parser_parse_func_call(ptcl_parser *parser, ptcl_parser_function *function, bool is_expression)
{
    ...
    if (function == NULL)
    {
        ...
        function = &instance_target->function;
    }
    ...
    while (!ptcl_parser_match(parser, ptcl_token_right_par_type))
    {
        ...
        ptcl_argument *argument = func_call.count < function->func.count ? &function->func.arguments[func_call.count] : NULL;
    ...
    {
        argument_count_mismatch = func_call.count < function->func.count - 1;
    }
    else
    {
        argument_count_mismatch = func_call.count != function->func.count;
    }
    ...
    func_call.return_type = function->func.return_type;
    if (function->is_built_in)
    {
        ptcl_expression *result = function->bind(parser, func_call.arguments, func_call.count, location, is_expression);
        if (parser->is_critical)
        {
            ptcl_statement_func_call_destroy(func_call);
            return (ptcl_statement_func_call){};
...
ptcl_type ptcl_parser_parse_type(ptcl_parser *parser, bool with_word, bool with_any)
{
    ...
    {
        with_syntax = ptcl_parser_parse_try_parse_syntax_usage_here(parser, false);
    }
...
ptcl_statement_typedata_decl ptcl_parser_parse_typedata_decl(ptcl_parser *parser, bool is_prototype)
{
    ...
    ptcl_parser_instance instance = ptcl_parser_typedata_create(parser->root, name, decl.members, decl.count);
...
{
    size_t start = parser->position;
    if (parser->is_syntax_body && ptcl_parser_parse_try_parse_syntax_usage_here(parser, false))
    {
        parser->position = start;
...
ptcl_expression *ptcl_parser_parse_unary(ptcl_parser *parser, bool only_value, ptcl_type *except, bool with_word)
{
        ...
        type != ptcl_binary_operator_less_equals_than_type)
    {
        ...
        if (type == ptcl_binary_operator_multiplicative_type)
        {
            ...
            if (value->return_type.type != ptcl_value_pointer_type || value->return_type.is_static)
            {
                                                   ...
                                                   ptcl_name name, ptcl_location location)
{
    ...
    ptcl_statement_func_call func_call = ptcl_parser_parse_func_call(parser, &target.function, true);
    if (parser->is_critical)
    {
        ptcl_expression_destroy(left);
        return NULL;
...
ptcl_expression *ptcl_parser_parse_value(ptcl_parser *parser, ptcl_type *except, bool with_word)
{
    ...
    case ptcl_token_left_curly_type:
        ...
        func.is_static = true;
        result = ptcl_expression_create(ptcl_expression_func_body_type, func, current.location);
        if (result == NULL)
        {
            ptcl_statement_func_body_destroy(body);
            break;
    ...
    case ptcl_token_word_type:
        ...
        if (ptcl_parser_current(parser).type == ptcl_token_left_par_type)
        {
            ...
            else
            {
                ...
                size_t start = parser->position;
                ptcl_statement_func_call func_call = ptcl_parser_parse_func_call(parser, func_instance, true);
                if (parser->is_critical)
                {
